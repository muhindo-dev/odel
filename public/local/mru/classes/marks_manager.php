...
class marks_manager {
     ...
     * @return array List of grade records ready for sync.
     */
    public function collect_course_grades(int $courseid): array {
        ...
        // Get all final grades for enrolled students with MRU mappings.
    ...
    public function sync_marks_to_core(int $courseid, ?int $initiatedby = null): stdClass {
        ...
        try {
            ...
            // Submit via API if configured.
            if ($this->api->is_configured()) {
                $response = $this->api->submit_marks($apimarks);
            }

            // Record each mark sync.
            $now = time();
            foreach ($marks as $mark) {
                try {
     ...
     * @return array List of sync records.
     */
     ...
     * @param int $processed Total records processed.
     * @param int $success Successful records.
     * @param int $failed Failed records.
     * @param string|null $errors Error summary.
     */
